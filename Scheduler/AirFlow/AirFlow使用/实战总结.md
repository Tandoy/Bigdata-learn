...
7.airflow读取配置文件
 ...
 7.3 选用 BashOperator 时配合 datax insert 到 clickhouse 时，如果ck表存在中文命名字段则需要进行 \`路程`\ 转移否则报错
 7.4 使用 datax 进行 Oracle到clickhouse的数据ETL时，当ck表字段类型是datetime 源数据必须大于 '1970-01-01 00:00:00' 否则插入失败