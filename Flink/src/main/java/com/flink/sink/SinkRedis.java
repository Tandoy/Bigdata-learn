...
public class SinkRedis {
    public static void main(String[] args) throws Exception {
        ...
        FlinkJedisPoolConfig jedisPoolConfig = new FlinkJedisPoolConfig.Builder()
                .setHost("dxbigdata103")
                .setPort(6379)
                .build();
        DataStreamSink<SensorReading> redisSink = mapOperator.addSink(new RedisSink<SensorReading>(jedisPoolConfig, new RedisMapper<SensorReading>() {
            @Override
            // 存储至redis命令，生成hash表
            public RedisCommandDescription getCommandDescription() {
                return new RedisCommandDescription(RedisCommand.HSET,"sink_redis_test");
            }

            @Override
            public String getKeyFromData(SensorReading sensorReading) {
                return sensorReading.getUid();
            }

            @Override
            public String getValueFromData(SensorReading sensorReading) {
                return sensorReading.getArea();
            }
        }));
        env.execute();
    }
